...
async fn dashboard_page_exposes_primary_operator_actions() {
    ...
    assert!(dashboard.contains("hx-get=\"/dashboard/latest-run\""));
...
#[tokio::test]
async fn dashboard_summary_route_renders_fragment() {
    let router = test_router().await;
...
#[tokio::test]
async fn dashboard_latest_run_route_renders_fragment() {
    let router = test_router().await;
