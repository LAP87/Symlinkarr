...
async fn request_library_count(client: &Client, cfg: &MediaBrowserConfig) -> Result<usize> {
    for endpoint in ["/Library/VirtualFolders/Query", "/Library/MediaFolders"] {
        let url = format!("{}{}", trim_base(&cfg.url), endpoint);
        let resp = send_with_retry(authenticated_request(client.get(&url), &cfg.api_key)).await?;
        ...
        if !resp.status().is_success() {
            ...
            bail!("Emby error {}: {}", status, body);
        }
        let value: serde_json::Value = resp.json().await?;
        return Ok(parse_folder_count(&value));
    }

    bail!("Emby library query endpoint not found")
}
...
pub(crate) async fn probe_libraries(cfg: &Config) -> Result<usize> {
    ...
    request_library_count(&client, &cfg.emby).await
}
...
pub(crate) async fn refresh_library_paths(
    ...
    if refresh_paths.is_empty() || !cfg.has_emby_refresh() {
        return Ok(telemetry);
    }

    let batches = build_update_batches(refresh_paths, cfg.emby.refresh_batch_size);
    let (batches, dropped_batches) =
        enforce_batch_limit(batches, cfg.emby.max_refresh_batches_per_run);
    ...
    if dropped_batches > 0 {
        if cfg.emby.abort_refresh_when_capped {
            telemetry.aborted_due_to_cap = true;
            telemetry.skipped_batches += telemetry.planned_batches;
            emit_refresh_line(
                emit_text,
                format!(
                    "   ⚠️  Emby: invalidation plan needed {} request(s), exceeding cap {}. Aborted all invalidation requests.",
                    telemetry.planned_batches, cfg.emby.max_refresh_batches_per_run
                ),
            );
            return Ok(telemetry);
        }
        ...
        emit_refresh_line(
            emit_text,
            format!(
                "   ⚠️  Emby: capped invalidation plan at {} request(s); {} request(s) skipped",
                cfg.emby.max_refresh_batches_per_run, dropped_batches
            ),
        );
    }

    let client = build_client();
    let delay = Duration::from_millis(cfg.emby.refresh_delay_ms);
    let batch_count = batches.len();
    let mut consecutive_failures = 0usize;

    for (idx, batch) in batches.into_iter().enumerate() {
        match post_media_updates(&client, &cfg.emby, &batch).await {
            Ok(()) => {
            ...
            Err(err) => {
                emit_refresh_line(
                    emit_text,
                    format!(
                        "   ⚠️  Emby: invalidation failed for {} path(s): {}",
                        batch.len(),
                        err
                    ),
                );
                    ...
                    && idx + 1 < batch_count
                {
                    ...
                    emit_refresh_line(
                        emit_text,
                        format!(
                            "   ⚠️  Emby: stopping remaining invalidation after {} consecutive batch failures; {} request(s) left unqueued",
    ...
    if telemetry.refreshed_batches > 0 {
        emit_refresh_line(
            emit_text,
            format!(
                "   📺 Emby: targeted invalidation queued for {} request(s) covering {} path(s)",
    ...
    if telemetry.skipped_batches > 0 {
        emit_refresh_line(
            emit_text,
            format!(
                "   ⚠️  Emby: {} invalidation request(s) were not queued",
